...
RECOG.FindStdGens_SL := function(sld,IsoDim)
...
RECOG.SLn_godownStingrayFinalVersion5:=function(list)
  ...
  while ocount < 100 do
      ...
      repeat
         ...
         degrees:=AsSortedList(List(factors,Degree));
         newdim:=first(degrees);
      until (count>20) or (newdim <> fail and (newdim < Maximum([Log2Int(d),3])));
      # Be careful if Log2Int(d) = 2
      ...
      # Check whether the stingray candidate is a stingray element
      check := RECOG.SmallCheckStingrayElement(rr,newdim,q);
      RECOG.ComparePPDFunction(r,d,q,newdim,Filtered(factors,x->Degree(x) = newdim));
      if check[1] then
...
RECOG.SLn_godown:=function(list)
  ...
  while ocount < 100 do
      ...
      if newdim > 2 then
          ...
          if check[1] then
              return [rr,newdim,check[2],check[3]];
          fi;
      else
          check := RECOG.SmallCheckStingrayElement(rr,newdim,q);
          if check[1] then
...
# Only works for newdim > 2  ?!?!?
# Are there no ppd stingray elements if newdim = 2?
RECOG.CheckStingrayElement:=function(rr,newdim,q)
    local fixbase, V, subspace, e, base, fld, r, module, b, action, n, factors, order, i, j, ppdcheck;
    ...
    module := GModuleByMats( [action], fld );
    if MTX.IsIrreducible(module) then
        factors := DuplicateFreeList(Factors(q^newdim-1));
        # TODO: Avoid to compute the order of r. Just check that there is
        # an factor a such that g^(a*factor) = 1.
        order := Order(r);
        for i in factors do
            ppdcheck := true;
            for j in [1..(newdim-1)] do
                if ((q^j-1) mod i = 0) then
                    ppdcheck := false;
                    break;
                fi;
            od;
            if ppdcheck and (order mod i = 0) then
                return [true,base,i];
            fi;
        od;
        return [false];
...
# input: list=[SL(d,q), d, q, SL(n,q)] acting as a subgroup of some big SL(n,q)
# output: list=[rr, dd] for a ppd(2*dd;q)-element rr
RECOG.SLn_godownFirstStingrayOld:=function(list)
  local d, first, q, g, gg, i, r, pol, factors, degrees, newdim, power, rr, ss,
  newgroup, colldegrees, exp, count, check, ocount;
  ...
  while ocount < 100 do
      ...
      # TODO: Dont use recursion. Instead use a loop in this function.
      if newdim > 2 then
          check := RECOG.CheckStingrayElementPPDListOld(rr,newdim,q);
          if check[1] then
              return [rr,newdim,check[2],check[3]];
          fi;
      else
...
# input: list=[SL(d,q), d, q, SL(n,q)] acting as a subgroup of some big SL(n,q)
# output: list=[rr, dd] for a ppd(2*dd;q)-element rr
RECOG.SLn_godownFirstStingray:=function(list)
  local d, first, q, g, gg, i, r, pol, factors, degrees, newdim, power, rr, ss,
  newgroup, colldegrees, exp, count, check, ocount;
  ...
  while ocount < 100 do
      ...
      # TODO: Dont use recursion. Instead use a loop in this function.
      if newdim > 2 then
          check := RECOG.CheckStingrayElementPPDList(rr,newdim,q);
          if check[1] then
              return [rr,newdim,check[2],check[3]];
          fi;
      else
...
# input: list=[SL(d,q), d, q, SL(n,q)] acting as a subgroup of some big SL(n,q)
# output: list=[rr, dd] for a ppd(2*dd;q)-element rr
RECOG.SLn_godownSecondStingray:=function(list,possiblePPDs,newdimsecondstingray)
  local d, first, q, g, gg, i, r, pol, factors, degrees, newdim, power, rr, ss,
  newgroup, colldegrees, exp, count, check, ocount;
  ...
  while ocount < 100 do
      ...
      # TODO: Dont use recursion. Instead use a loop in this function.
      if newdim > 2 then
          check := RECOG.CheckStingrayElementPossiblePPDs(rr,newdim,q,possiblePPDs,newdimsecondstingray);
          if check[1] then
              return [rr,newdim,check[2],check[3]];
          fi;
      else
...
# Only works for newdim > 2  ?!?!?
# Are there no ppd stingray elements if newdim = 2?
RECOG.CheckStingrayElementPPDListOld:=function(rr,newdim,q)
    local fixbase, V, subspace, e, base, fld, r, module, b, action, n, factors, order, i, j, ppdcheck, ppdlist, d;
    ...
    if MTX.IsIrreducible(module) then
        ...
        # an factor a such that g^(a*factor) = 1.
        order := Order(r);
        factors := DuplicateFreeList(Factors(order));
        ppdlist := [];
        for i in factors do
            for e in [1..n] do
                #(q^e mod i = 0)
        ...
        if IsEmpty(ppdlist) then
            return [false];
        else
            # d := Size(r);
            # Apply(ppdlist,x->d-x);
            # Apply(ppdlist,x->newdim-x);
            # ppdlist := Filtered(ppdlist,x->IsPrime(x));
            # if ppdlist = [] then
            #     return [false];
            # else
            #     return[true,base,ppdlist];
            # fi;
            return[true,base,ppdlist];
        fi;
    else
...
# Only works for newdim > 2  ?!?!?
# Are there no ppd stingray elements if newdim = 2?
RECOG.CheckStingrayElementPPDList:=function(rr,newdim,q)
    local fixbase, V, subspace, e, base, fld, r, module, b, action, n, factors, order, i, j, ppdcheck, ppdlist, d;
...
# Only works for newdim > 2  ?!?!?
# Are there no ppd stingray elements if newdim = 2?
RECOG.CheckStingrayElementPossiblePPDs:=function(rr,newdim,q,possiblePPDs,newdimsecondstingray)
    local fixbase, V, subspace, e, base, fld, r, module, b, action, n, factors, order, i, j, ppdcheck, ppdlist, d, cn, e2;
    
    fld := GF(q);
    r := StripMemory(rr);
    n := Size(r);
    # cn := newdim + newdimsecondstingray;
    # Apply(possiblePPDs,x->cn-x);
    # possiblePPDs := Filtered(possiblePPDs,x-> x>0);
    # possiblePPDs := Filtered(possiblePPDs,x->IsPrime(x));
    # if possiblePPDs = [] then
    #     return [false];
    # fi;
    fixbase := NullspaceMat(r-IdentityMat(n,fld));
    ...
    if MTX.IsIrreducible(module) then
        ...
        # Maybe use cn here instead of n?
        Apply(possiblePPDs,x->[x[1],n-x[2]] );
        # cn := newdim + newdimsecondstingray;
        # Apply(possiblePPDs,x->[x[1],cn-x[2]] );
        # possiblePPDs := Filtered(possiblePPDs,x-> x[2]>0);
        
        
        # CONTINUE HERE. Whats the problem for the second ppd element?
        
        for i in factors do
            for e in possiblePPDs do
                e2 := e[2];
                if ((q^e2)-1 mod i = 0) then
                    ppdcheck := true;
                    for j in [1..(e2-1)] do
                        if ((q^j-1) mod i = 0) then
                            ppdcheck := false;
...
# output is a group element acting irreducibly in two dimensions, and fixing
# a (dimension-2)-dimensional subspace
# uses two stingray elements as in the paper by cheryl praeger and alice niemeyer
RECOG.SLn_constructppd2TwoStingray:=function(g,dim,q)
  local out, list, out2;
...
# a (dimension-2)-dimensional subspace
# uses two stingray elements as in the paper by cheryl praeger and alice niemeyer
# Also checks that ppd(g,e1), ppd(g,e2) fulfills e1 + e2 = d
RECOG.SLn_constructppd2TwoStingrayVersion2:=function(g,dim,q)
  local out, list, out2;
  ...
  repeat
     ...
     Print(list[2]);
     Print("\n");
     out:=RECOG.SLn_godownFirstStingrayOld(list);
     if out=fail or out[1]*out[1]=One(out[1]) then
        if InfoLevel(InfoRecog) >= 3 then Print("B\c"); fi;
        list:=[g,dim,q,g];
        out:=fail;
...
# uses two stingray elements as in the paper by cheryl praeger and alice niemeyer
# Also checks that ppd(g,e1), ppd(g,e2) fulfills e1 + e2 = d
RECOG.SLn_constructppd2TwoStingrayVersion3:=function(g,dim,q)
  local out, list, out2;
  ...
  repeat
     Print("again.\n");
     out:=RECOG.SLn_godownFirstStingray(list);
     Display(out[2]);
     if out=fail or out[1]*out[1]=One(out[1]) then
        if InfoLevel(InfoRecog) >= 3 then Print("B\c"); fi;
        list:=[g,dim,q,g];
        ...
        if out[2]>2 then
           repeat
                out2:=RECOG.SLn_godownFirstStingray(list);
                if out2=fail or out2[1]*out2[1]=One(out2[1]) then
                    if InfoLevel(InfoRecog) >= 3 then Print("B\c"); fi;
                    list:=[g,dim,q,g];
                    out2:=fail;
...
# uses two stingray elements as in the paper by cheryl praeger and alice niemeyer
# Also checks that ppd(g,e1), ppd(g,e2) fulfills e1 + e2 = d
RECOG.SLn_constructppd2TwoStingrayVersion4:=function(g,dim,q)
  local out, list, out2, currentdim;
...
RECOG.SLn_UpStep := function(w)
  ...
  Print("\n");
  Print("New dimension: ");
  Print(Minimum(2*w.n-1,w.GoalDim));
  Print("\n");
...
#################################################################################
## Constructive Recognition of SL(4,q) by Leedham-Green and O'Brien algorithm ###
#################################################################################
...
RECOG.FindStdGens_Sp := function(sld,IsoDim)
...
RECOG.CheckNewStingrayGroupSp := function(g1,dim1,base1,g2,dim2,base2,q)
    local baseSum, b, action1, action2, fld, module;
...
#
#    # F := SplittingField(f);
#    # roots := RootsOfPolynomial(F,f);
#    roots := RootsOfUPol( "split", f );
#    for ele in roots do
#        if ele^(-1) in roots then
#            return true;
#        fi;
#    od;
#
...
RECOG.FindStdGens_SO := function(sld,IsoDim)
...
RECOG.SOn_constructso2:=function(g,d,q,form)
  ...
  #r:=RECOG.SLn_constructppd2TwoStingray(g,d,q);
  #r:=RECOG.SLn_constructppd2TwoStingrayVersion2(g,d,q);
  r := RECOG.SOn_constructppdTwoStingray(g,d,q,form);
  Print("Finished main GoingDown, i.e. we found a stringray element which operates irreducible on a 2 dimensional subspace. \n");
  Print("Next goal: Find an random element s.t. the two elements generate SO(4,q). \n");
  Error("here");
  # Remark D.R.: at this point we know that h is isomorphic to SO(4,q)
  Print("Succesful. ");
  Print("Current Dimension: 4\n");
  Print("Next goal: Generate SL(2,q). \n");
  if not (q in [2,3,4,5,9]) then
...
##############################################################################
# The GoingDown method for SO:
##############################################################################

# TODO: Work on comments and documentation

RECOG.CheckNewStingrayGroupSO := function(g1,dim1,base1,g2,dim2,base2,q)
    local baseSum, b, action1, action2, fld, module;
...
# Only works for newdim > 2  ?!?!?
# Are there no ppd stingray elements if newdim = 2?
RECOG.SmallCheckStingrayElementSO := function(rr,newdim,q,form)
    local fixbase, V, subspace, e, base, fld, r, module, b, action, n, factors, order, i, j, ppdcheck, restricted;
...
# input: list=[SL(d,q), d, q, SL(n,q)] acting as a subgroup of some big SL(n,q)
# output: list=[rr, dd] for a ppd(2*dd;q)-element rr
RECOG.SOn_godownStingrayVersion1:=function(list)
  local d, CheckFactors, CheckOtherFactors, q, p, g, gg, i, r, pol, factors, degrees, newdim, power, rr, ss,
  newgroup, colldegrees, exp, count, check, ocount, beta, NiList, Maxi, form;
...
# input: list=[SL(d,q), d, q, SL(n,q)] acting as a subgroup of some big SL(n,q)
# output: list=[rr, dd] for a ppd(2*dd;q)-element rr
RECOG.SOn_godownStingrayVersion2:=function(list)
  local d, first, q, p, g, gg, i, r, pol, factors, degrees, newdim, power, rr, ss,
  newgroup, colldegrees, exp, count, check, ocount, beta, NiList, Maxi, qFactors, irrfact, invbase, form, CheckOtherFactors, CheckFactors, fld, restricted, b, j;
  ...
  while ocount < 100 do
      ...
      until (count>200) or (newdim <> fail and (degrees[newdim] < Maximum([2*Log2Int(d),3])));
      # Be careful if Log2Int(d) = 2! In this case we search for stingray elements with k < 2. Hence use newdim < Maximum([Log2Int(d),3])
      # 2*Log2Int(d)-2 to improve 2*Log2Int(8) = 6 -> 2x 4 dimensional => again 8 dimensional
...
RECOG.SOn_constructppdTwoStingray:=function(g,dim,q,form)
  local out, list, out2, currentdim;
  ...
  currentdim := dim;
  repeat
     out:=RECOG.SOn_godownStingrayVersion2(list);
     if out=fail or out[1]*out[1]=One(out[1]) then
        if InfoLevel(InfoRecog) >= 3 then Print("B\c"); fi;
        Print("Restart. \n");
        Print("Current Dimension: ");
        ...
        if out[2]>=2 then
           repeat
                out2:=RECOG.SOn_godownStingrayVersion2(list);
                if out2=fail or out2[1]*out2[1]=One(out2[1]) then
                    if InfoLevel(InfoRecog) >= 3 then Print("B\c"); fi;
                    list:=[g,dim,q,g,form];
                    out2:=fail;
           ...
           until out2<>fail and out2[2] >= 2;
           if RECOG.CheckNewStingrayGroupSO(out[1],out[2],out[3],out2[1],out2[2],out2[3],q) then
                list:=[Group(out[1],out2[1]),out[2]+out2[2],q,g,form];
                currentdim := list[2];
